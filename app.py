...
def import_file_glob(relative_pattern):
    ...
    Handles spaces in folder names by trying multiple path formats.
    """
...
if missing_critical:
    ...
    st.write("### Available Python files in project:")
    py_files = glob.glob(os.path.join(BASE_DIR, "**/*.py"), recursive=True)
    for f in sorted(py_files):
        st.write(f"  • {os.path.relpath(f, BASE_DIR)}")
    st.stop()
...
def load_inflation_data_ui(grain_code, ppy, page_key):
    """UI for loading and interpolating inflation data"""
...
def load_discounting_data_ui(grain_code, ppy, page_key):
    """UI for loading discounting data (yield curve or flat rate)"""
...
def navigate_to(page, breadcrumb_label=None):
    """Navigate to a new page with optional breadcrumb"""
...
def render_home():
    ...
    with col1:
        ...
        <div class="card">
            ...
            <p>Complete PAA valuation with Income Statement & Liability Rollforward</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        st.markdown("""
        <div class="card">
            <h3>📊 LRC Calculators</h3>
            <p>UPR (365th/24th/8th) & Loss Component (Onerous Contracts)</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col3:
        st.markdown("""
        <div class="card">
            <h3>📉 LIC Calculators</h3>
            <p>IBNR • OCR • ULAE • NPR • Risk Adjustment</p>
        </div>
        """, unsafe_allow_html=True)
...
def render_lrc():
    ...
    st.markdown("""
    <div class="hero">
        <h1>LRC Calculators</h1>
        <p>Liability for Remaining Coverage — IFRS 17 PAA</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        st.markdown("""
        <div class="card">
            <h3>📅 UPR Calculator</h3>
            <p>Unearned Premium Reserve — Pro-rata calculation using 365th, 24th, or 8th methods</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        st.markdown("""
        <div class="card">
            <h3>⚠️ Loss Component</h3>
            <p>Onerous contract identification — Loss Component = max(Combined Ratio - 1, 0) × UPR</p>
        </div>
        """, unsafe_allow_html=True)
...
def render_lic():
    ...
    st.markdown("""
    <div class="hero">
        <h1>LIC Calculators</h1>
        <p>Liability for Incurred Claims</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        st.markdown("""
        <div class="card">
            <h3>💰 Fulfilment Cashflows</h3>
            <p>OCR • IBNR (4 methods) • ULAE • NPR</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        <div class="card">
            ...
            <p>Mack Chain Ladder • ODP Bootstrap</p>
        </div>
        """, unsafe_allow_html=True)
...
def render_fulfilment_cashflows():
    ...
    st.markdown("""
    <div class="hero">
        <h1>Fulfilment Cashflows</h1>
        <p>Components of LIC</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    items = [
        ...
        ("🛡️ NPR", "npr_calculator", npr_engine, "Reinsurance Non-Performance Risk"),
    ]
    ...
    for i, (title, page, module, desc) in enumerate(items):
        with cols[i]:
            ...
            """, unsafe_allow_html=True)
            if st.button(f"Open {title.split()[1]}", key=f"nav_fcf_{page}", 
                        disabled=not available, use_container_width=True):
                navigate_to(page, ['Home', 'LIC Calculators', 'Fulfilment Cashflows', title.split()[1]])
    
    back_button('lic', ['Home', 'LIC Calculators'])


# =============================================================================
#  PAGE: IBNR METHODS MENU
# =============================================================================
...
def render_ibnr_menu():
    ...
    methods = [
        ("📊 Percentage Method", "percentage_calculator", ibnr_pct, 
         "Simple: IBNR = Amount × IBNR%"),
        ("🔗 Basic Chain Ladder", "bcl_calculator", ibnr_bcl,
         "Multi-LDF: Vol-Weighted, Simple Avg, Geometric, Medial, Regression, Wtd Last 3"),
        ("🎯 Cape Cod", "capecod_calculator", ibnr_cc,
         "Uses premiums to derive expected loss ratio"),
        ("📈 Bornhuetter-Ferguson", "bf_calculator", ibnr_bf,
         "Blends Chain Ladder with expected loss ratio"),
    ]
        ...
        for j in range(2):
            if i + j < len(methods):
                ...
                with cols[j]:
                    ...
                    """, unsafe_allow_html=True)
                    if st.button(f"Open {name.split(' ', 1)[1]}", key=f"nav_ibnr_{page}",
                                disabled=not available, use_container_width=True):
                        navigate_to(page, ['Home', 'LIC Calculators', 'Fulfilment Cashflows', 
                                          'IBNR Methods', name.split(' ', 1)[1]])
    
    back_button('fulfilment_cashflows', ['Home', 'LIC Calculators', 'Fulfilment Cashflows'])


# =============================================================================
#  PAGE: RISK ADJUSTMENT
# =============================================================================
...
def render_risk_adjustment():
    ...
    <div class="hero">
        ...
        <p>RA Methods for IFRS 17</p>
    </div>
    """, unsafe_allow_html=True)
    
    cols = st.columns(2)
    methods = [
        ("📐 Mack Chain Ladder", "mack_calculator", mack_engine,
         "Distribution-free standard error of IBNR (Mack 1993)"),
        ("🎲 ODP Bootstrap", "bootstrap_calculator", bootstrap_engine,
         "England & Verrall bootstrap with process variance"),
    ]
    ...
    for i, (name, page, module, desc) in enumerate(methods):
        with cols[i]:
            ...
            """, unsafe_allow_html=True)
            if st.button(f"Open {name.split(' ', 1)[1]}", key=f"nav_ra_{page}",
                        disabled=not available, use_container_width=True):
                navigate_to(page, ['Home', 'LIC Calculators', 'Risk Adjustment', name.split(' ', 1)[1]])
...
def render_upr_calculator():
    ...
    st.markdown("""
    <div class="hero">
        <h1>UPR Calculator</h1>
        <p>Unearned Premium Reserve — Pro-rata Methods</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.container():
        ...
        with c4:
            st.markdown("<br>", unsafe_allow_html=True)
            st.caption(f"Method: {'Exact daily pro-rata' if method == '365th' else 'Half-month approximation' if method == '24th' else 'Half-quarter approximation'}")
    ...
    uploaded_file = st.file_uploader(
        "📂 Upload Premium Register (CSV or Excel)",
        type=["csv", "xlsx", "xls"],
        key="upr_f"
    )
    ...
    if uploaded_file is not None:
        try:
            ...
            numeric_columns = [
                ...
                if c not in [start_date_col, end_date_col] + grouping_cols 
                and pd.api.types.is_numeric_dtype(df[c])
            ]
            selected_value_cols = st.multiselect(
                "Amount Columns (to apply UPR)",
            ...
            if len(bad_dates) > 0:
                st.warning(f"⚠️ {len(bad_dates)} rows have End_Date ≤ Start_Date and will be excluded.")
            ...
            if st.button("🔢 Calculate UPR", key="upr_calc", use_container_width=True):
                with st.spinner("Calculating UPR..."):
                    ...
                    df_processed['Remaining_Days'] = np.clip(
                        df_processed['Remaining_Days'], 0, df_processed['Duration_Days']
                    )
                    
                    if method == "365th":
                        df_processed['Unearned_Portion'] = (
                            df_processed['Remaining_Days'] / df_processed['Duration_Days']
                        )
                    elif method == "24th":
                        interval = 365.25 / 24
                        df_processed['Unearned_Portion'] = (
                            (df_processed['Remaining_Days'] / interval) / 
                            (df_processed['Duration_Days'] / interval)
                        )
                    else:  # 8th
                        interval = 365.25 / 8
                        df_processed['Unearned_Portion'] = (
                    ...
                    for c in selected_value_cols:
                        df_processed[f"{c}_UPR"] = (
                            df_processed['Unearned_Portion'] * df_processed[c]
                        )
                ...
                st.download_button(
                    "⬇ Download UPR Results (Excel)",
                    data=output,
                    file_name=f"{sc}_{so}_UPR_{method}.xlsx",
...
def render_loss_component():
    ...
    <div class="hero">
        ...
        <p>Onerous Contract Identification — IFRS 17 PAA</p>
    </div>
    """, unsafe_allow_html=True)
            ...
            if st.button("🔢 Calculate Loss Component", key="lc_run", use_container_width=True):
                ...
                with st.spinner("Calculating Loss Component..."):
                    result = loss_comp_engine.calculate_loss_component(
                        df=df,
                        lob_col=lob_col,
                        written_premium_col=wp_col,
                        expenses_col=expenses_col,
                        commission_col=commission_col,
                        paid_claims_col=paid_claims_col,
                        opening_ocr_col=opening_ocr_col,
                        closing_ocr_col=closing_ocr_col,
                        opening_ibnr_col=opening_ibnr_col,
                        closing_ibnr_col=closing_ibnr_col,
                        opening_upr_col=opening_upr_col,
                        closing_upr_col=closing_upr_col,
                ...
                sc = sanitize_filename(client_name)
                st.download_button(
                    "⬇ Download Loss Component Results",
                    data=output,
                    file_name=f"{sc}_Loss_Component.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    key="lc_dl"
                )
...
def render_ocr_calculator():
    ...
    <div class="hero">
        ...
        <p>Outstanding Claims Reserve — Group & Sum</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    uploaded_file = st.file_uploader(
        "📂 Upload Case Estimates File (CSV or Excel)",
        type=["csv", "xlsx", "xls"],
        key="ocr_f"
    )
...
def render_percentage_calculator():
    ...
    st.markdown("""
    <div class="hero">
        <h1>Percentage IBNR Calculator</h1>
        <p>Simple Method: IBNR = Amount × IBNR Percentage</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with c2:
        ibnr_pct_val = st.number_input(
            "IBNR Percentage (%)", 0.0, 100.0, 10.0, 0.5, key="pct_pct"
        ) / 100.0
    ...
    try:
        ...
        amount_candidates = [
            c for c in cols 
            if c not in [date_col, lob_col] 
            and pd.api.types.is_numeric_dtype(df[c])
        ]
        amount_cols = st.multiselect(
            "Amount Column(s)", amount_candidates, key="pct_amt"
        )
        ...
        if st.button("🔢 Calculate Percentage IBNR", key="pct_run", use_container_width=True):
            ...
            <div class="metric-card">
                <div class="label">Total IBNR @ {ibnr_pct_val:.1%}</div>
            ...
            sc = sanitize_filename(client_name)
            st.download_button(
                "⬇ Download Percentage IBNR Results",
                data=output,
                file_name=f"{sc}_Percentage_IBNR.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                key="pct_dl"
            )
...
def render_bcl_calculator():
    ...
    st.markdown("""
    <div class="hero">
        <h1>Basic Chain Ladder (BCL) — IBNR</h1>
        <p>Multi-LDF Methods with Inflation & Discounting Support</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    uploaded = st.file_uploader(
        "📂 Upload Claims Triangle Data (CSV/Excel: Loss Date, Report Date, LOB, Amount)",
        type=["csv", "xlsx", "xls"],
        key="bcl_f"
    )
    ...
    try:
        ...
        if st.button("🔢 Calculate BCL IBNR", key="bcl_run", use_container_width=True):
            ...
            with st.spinner("Calculating BCL IBNR..."):
                ...
                # Show all LDFs for first amount column
                sample_amt = amount_cols[0]
                _, sample_cum, _ = engine_utils.build_triangles(
                    df, loss_col, rep_col, sample_amt, from_dt, grain_code, n_periods
                )
                ...
                for lob in lobs:
                    ...
                    for ac in amount_cols:
                        _, cum, _ = engine_utils.build_triangles(
                            lob_data, loss_col, rep_col, ac, from_dt, grain_code, n_periods
                        )
            ...
            sc = sanitize_filename(client_name)
            st.download_button(
                "⬇ Download BCL Results",
                data=output,
                file_name=f"{sc}_BCL_IBNR.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                key="bcl_dl"
            )
...
def render_capecod_calculator():
    ...
    <div class="hero">
        <h1>Cape Cod — IBNR</h1>
        <p>Uses premiums to derive expected loss ratio</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    prem_file = st.file_uploader(
        "📂 Premiums Data (LOB, Premium Amount)",
        type=["csv", "xlsx", "xls"],
        key="cc_pf"
    )
    ...
    try:
        ...
        if st.button("🔢 Calculate Cape Cod IBNR", key="cc_run", use_container_width=True):
            ...
            with st.spinner("Calculating Cape Cod IBNR..."):
                ...
                for lob in lobs:
                    ...
                    for yr in range(from_dt.year, from_dt.year + n_periods):
                        yr_prem = prem_sub[prem_sub.apply(
                            lambda r: str(yr) in str(r[prem_lob_col]) if pd.notna(r[prem_lob_col]) else False,
                            axis=1
                        )][prem_amt_col].sum()
                    ...
                    for ac in amount_cols:
                        ...
                        # Check which Cape Cod function signature is available
                        try:
                ...
                # Find IBNR column
                ibnr_col = next((c for c in final_df.columns if 'IBNR' in c), 'Cape_Cod_IBNR')
            ...
            sc = sanitize_filename(client_name)
            st.download_button(
                "⬇ Download Cape Cod Results",
                data=output,
                file_name=f"{sc}_CapeCod_IBNR.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                key="cc_dl"
            )
...
def render_ulae_calculator():
    ...
    st.markdown("""
    <div class="hero">
        <h1>ULAE Calculator</h1>
        <p>Unallocated Loss Adjustment Expenses — Paid-to-Paid Method</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    uploaded = st.file_uploader(
        "📂 Upload Reserves File (LOB, OCR, IBNR)",
        type=["csv", "xlsx", "xls"],
        key="ulae_f"
    )
    ...
    try:
        ...
        if st.button("🔢 Calculate ULAE", key="ulae_run", use_container_width=True):
            ...
            sc = sanitize_filename(client_name)
            st.download_button(
                "⬇ Download ULAE Results",
                data=output,
                file_name=f"{sc}_ULAE.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                key="ulae_dl"
            )
...
def render_npr_calculator():
    ...
    st.markdown("""
    <div class="hero">
        <h1>NPR Calculator</h1>
        <p>Reinsurance Non-Performance Risk — IFRS 17 Para 63(e)</p>
    </div>
    """, unsafe_allow_html=True)
        ...
        if st.button("🔢 Calculate NPR", key="npr_run", use_container_width=True):
            ...
            for _, ri in ri_df.iterrows():
                for _, lic in lic_df.iterrows():
                    ...
                    rows.append({
                        'Reinsurer': ri[name_col],
                        'Portfolio': lic[port_col],
                        'PD': ri[pd_col],
                        'Share': ri[share_col],
                        'Total_LIC': lic['Total_LIC'],
                        'NPR': npr
                    })
            ...
            sc = sanitize_filename(client_name)
            st.download_button(
                "⬇ Download NPR Results",
                data=output,
                file_name=f"{sc}_NPR.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                key="npr_dl"
            )
...
def render_mack_calculator():
    ...
    st.markdown("""
    <div class="hero">
        <h1>Mack Chain Ladder — Risk Adjustment</h1>
        <p>Distribution-free standard error of IBNR (Mack 1993)</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with c2:
        confidence = st.number_input(
            "Confidence Level (%)", 50.0, 99.9, 75.0, 1.0, key="mck_cl"
        ) / 100.0
    ...
    uploaded = st.file_uploader(
        "📂 Upload Cumulative Claims Triangle (CSV/Excel — Rows=AY, Columns=Dev)",
        type=["csv", "xlsx", "xls"],
        key="mck_f"
    )
    
    st.caption("Expected format: First column = Accident Year labels, remaining columns = cumulative claims by development period.")
    ...
    try:
        ...
        if st.button("🔢 Calculate Mack RA", key="mck_run", use_container_width=True):
                ...
                for i in range(n_ay):
                    ...
                    for j in range(n_dev - 1, -1, -1):
                        if i + j < n_ay and not np.isnan(C[i, j]):
                            last_obs = j
                            break
                    if last_obs < 0 or last_obs >= n_dev - 1:
                        continue
            ...
            st.markdown(f"### 📊 Mack RA Results @ {confidence:.0%} Confidence")
            ...
            sc = sanitize_filename(client_name)
            st.download_button(
                "⬇ Download Mack Results",
                data=output,
                file_name=f"{sc}_Mack_RA.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                key="mck_dl"
            )
...
def render_bootstrap_calculator():
    ...
    st.markdown("""
    <div class="hero">
        <h1>ODP Bootstrap — Risk Adjustment</h1>
        <p>England & Verrall (1999) Bootstrap with Process Variance (Gamma ODP)</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with c2:
        confidence = st.number_input(
            "Confidence Level (%)", 50.0, 99.5, 75.0, 1.0, key="bts_cl"
        ) / 100.0
    with c3:
    ...
    st.caption("Expected format: First column = Accident Year labels, remaining = cumulative claims by development period.")
    ...
    try:
        ...
        if st.button(f"🎲 Run Bootstrap ({n_iter:,} iterations)", key="bts_run", use_container_width=True):
            with st.spinner(f"Running {n_iter:,} bootstrap iterations..."):
                ...
                # Fitted increments and residuals
            ...
            st.markdown(f"### 🎲 Bootstrap Results @ {confidence:.0%} Confidence")
            ...
            st.caption(f"Φ (Scale Parameter): {phi:.4f} | Iterations: {n_iter:,} | Process Variance: {'Yes' if add_pv else 'No'}")
            
            # Histogram
            st.markdown("#### IBNR Distribution")
            hist_data = pd.DataFrame({'IBNR': arr})
            st.bar_chart(pd.cut(arr, bins=30).value_counts().sort_index())
...
# =============================================================================
#  CALCULATOR: FULL IFRS 17 VALUATION
# =============================================================================
...
def render_full_valuation():
    ...
    st.markdown("""
    <div class="hero">
        <h1>Full IFRS 17 Valuation</h1>
        <p>Complete PAA LRC Rollforward — Income Statement & Liability Movement</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.warning("🚧 Full Valuation module is under active development. Basic functionality available.")
            ...
            if st.button("🔢 Run Full Valuation", key="fv_run", use_container_width=True):
                ...
                for group, data in results.items():
                    with st.expander(f"📁 {group}", expanded=True):
                        ...
                        st.markdown(f"""
                        **Combined Ratio:** {data.get('Combined_Ratio', 0):.2%}  
                        **Locked-in Rate:** {data.get('Locked_In_Rate', 0):.4%}  
                        **UPR Snapshot:** {data.get('UPR_Snapshot', 0):,.2f}
                        """)
